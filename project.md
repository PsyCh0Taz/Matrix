...
- Un flux possède un statut modifiable limité aux valeurs ONLINE, OFFLINE, PENDING et CRASHED.
- Le statut par défaut d'un nouveau flux est PENDING.
- Dans la fenêtre ouverte depuis une connexion cartographique, un clic sur le statut d'un flux le fait avancer immédiatement dans le cycle ONLINE, PENDING, OFFLINE, CRASHED, puis ONLINE.
...
- Chaque service est affiché avec la couleur de son statut. Chaque connexion reprend la couleur du statut le plus bloquant parmi ses services et ses flux regroupés, selon l'ordre ONLINE, PENDING, OFFLINE, CRASHED.
...
- Des filtres « statut » portent sur les statuts ONLINE, OFFLINE, PENDING et CRASHED des équipements, des services et des flux.
...
- Le fichier SVG exporté embarque ses styles et le thème courant afin de conserver les couleurs des zones, équipements, services et connexions lorsqu'il est ouvert indépendamment de l'application.
